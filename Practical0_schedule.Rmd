...
You should have a 'search' bar at the bottom left of your NUIT Windows PC, labelled _Type here to search_. Alternatively, click on the Windows button at the bottom list. Type **_RStudio_** and the launch icon should be displayed for you to click on:

```{r, echo = FALSE}
knitr::include_graphics("images/start_from_windows_menu.png")
```

Do **not** click on the icon labelled _R i386_ or _R x64_ )


### **Pop-up windows on NUIT PCs**
You may see one or more irritating pop-up windows when you first use RStudio:

* If asked which version, click "OK" to select the default
* If asked to send back Usage Information click "No"
* If asked to update RStudio click "Ignore Update"
...
Make sure that the following boxes are **unticked** or changed (see **red box** below):
...
## How to install and activate a package from CRAN (not needed in today's session, but important to read)**
...
## Installing an R package **(not needed in NUIT cluster room)**
R has a special function called `install.packages()` that allows you download software from the internet onto your PC. The packages you use in NES2303 should already be installed, so you **do not** need to do this step. If working at home on your own laptop or MacBook you need to install the package (**unsupported**). For home laptops / MacBooks the instructions are below.

On the left of your RStudio screen you should see a large window headed **Console** which will contain introductory text about the R version you are using etc. On my PC it shows as version 4.0.2 but you may have a more recent version:
...
## Activating an R package **(needed for every practical, including NUIT clusters)**
...
You will probably see lots of red text displayed warning you that various 'objects are masked'. This is the output that I see:

```{r, eval=TRUE, echo=FALSE}
detach("package:mosaic", unload = TRUE)
library(mosaic)
```
This is simply to tell you that the `mosaic` package loads additional commands, that have the same names as some basic ones, and is nothing to worry aboue.


# RStudio projects (**Important**)
...
Go back to your R script (top left window in RStudio) and enter the following code to create a vector of animal weights. You can hit Ctrl-Enter to run the line:
...
We have already looked at importing CSV files, which you can export from Microsoft Excel using Excel's "Save As" option. This makes them the most useful "general" format for storing data. The general structure is:

```{r, eval = FALSE, echo = TRUE}
# Import data from a CSV file (don't run next line)
my_data_object <- read.csv(file = "Path_to_the_file.csv")
# Which you ran earlier as
pract0_dat <- read.csv(file = "yield.csv")
# or if you put yield in a Data folder
pract0_dat <- read.csv(file = "Data/yield.csv")
```

Notice how the words `file =` are optional. You can just put the filename in quotes, but it is good practice to include the `file =` part as it makes your code easier to read.
...
```{r, eval = FALSE, echo = TRUE}
# writing my_data_object
# Edit the next line to save your animals_df object into a file called my_output_data.csv
...
## Books, online guides
See the Canvas reading list for NES2303. I have updated this with new resources for this year, with most books in electronic format. There are also lots of excellent resources out there for learning R, some of those that we recommend are listed here and should be enough to keep you going 

[Our Coding Club - written by biology students for biology students](https://ourcodingclub.github.io/)
